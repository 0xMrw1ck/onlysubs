...
ipcMain.handle('pick-export-folder',async event=>{
 ...
 if(!win)throw new Error('This request must come from the onlysubs window.')
 const chosen=await dialog.showOpenDialog(win,{title:'Choose where onlysubs exports videos',properties:['openDirectory','createDirectory']})
