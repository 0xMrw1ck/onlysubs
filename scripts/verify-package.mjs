...
for(const name of ['electron/main.cjs','server/api.cjs','server/start.cjs','server/security.cjs','shared/policies.mjs'])assert.deepEqual(asar.extractFile(file,name),await fs.readFile(name),name+' must match tested source')
assert.equal(JSON.parse(asar.extractFile(file,'package.json')).version,'0.2.0')
for(const name of ['LICENSE.electron.txt','LICENSES.chromium.html','THIRD-PARTY-NOTICES.txt','resources/app.asar.unpacked/server/transcribe.py'])await fs.access(path.join(root,name))
