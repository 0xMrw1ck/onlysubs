...
const pkg=JSON.parse(await fs.readFile('package.json','utf8')),name=`onlysubs-${pkg.version}-setup-x64.exe`
...
for(const name of ['latest.yml',`onlysubs-${pkg.version}-setup-x64.exe.blockmap`])await fs.copyFile(path.join(path.dirname(source),name),path.join('outputs/windows',name))
