...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  const win=new BrowserWindow({show:false,webPreferences:{preload:path.resolve(__dirname,'../electron/preload.cjs'),nodeIntegration:false,contextIsolation:true,sandbox:true}})
  ...
  const result=await win.webContents.executeJavaScript(`new Promise(resolve=>{let attempts=0;const timer=setInterval(()=>{const dialog=document.querySelector('dialog[open]');if(dialog||++attempts>30){clearInterval(timer);resolve({title:document.title,dialog:dialog?.textContent,buttons:document.querySelectorAll('button').length,nodeAccess:typeof window.require,folderPicker:typeof window.studio?.pickExportFolder})}},100)})`)
  if(!result.dialog?.includes('Welcome to onlysubs')||result.nodeAccess!=='undefined'||result.folderPicker!=='function')throw Error('Desktop UI smoke failed: '+JSON.stringify(result))
